...
'''
    ...
    algorithm in PyTorch Lightning. Inspired by:
    ...
    - Phil Tabor's implementation of TD3: https://github.com/philtabor/Youtube-Code-Repository/tree/master/ReinforcementLearning/PolicyGradient/TD3
...
class TD3(LightningModule):
    ...
    def __init__(self,
        ...
        self.q1_net = None
        self.target_q1_net = None
        self.q2_net = None
        self.target_q2_net = None
        self.build_networks()
    ...
    def build_networks(self)->None:
        ...
        self.target_q2_net = MLP(mlp_type = "q_net",
                         state_space_dim = self.state_space_dim,
                         action_space_dim = self.action_space_dim,
                         **self.hparams.q_net_arch_dict)
        ...
        self.target_policy = MLP(mlp_type = "policy",
                                 state_space_dim = self.state_space_dim,
                                 action_space_dim = self.action_space_dim,
                                 **self.hparams.policy_arch_dict)
    ...
    def forward(self, state: th.Tensor) -> Tuple[th.Tensor, th.Tensor]:
        ...
        min_q_value = th.min(q1_value, q2_value)
        return min_q_value, action
    ...
    def configure_optimizers(self) -> List[Optimizer]:
        ...
        # Pass both critic nets' params to optimizer constructor
        critic_optim = self.hparams.q_net_optimizer_class(list(self.q1_net.parameters())\
    ...
    def training_step(self, batch: Tuple[Tensor, Tensor], _) -> OrderedDict:
        ...
        q1_values = self.q1_net.forward(states_, actions_)
        q2_values = self.q2_net.forward(states_, actions_)
        
        # Compute next target Q-values
        with th.no_grad():
            
            # Compute next actions with target policy
            actions_next_ = self.target_policy.forward(states_next_)
            
            # Add noise term
            actions_next_ = th.clamp(actions_next_ + th.Tensor(self.action_noise_gen()).to(actions_next_.device),
                                     min=-1.0, max=1.0
                                    )
            
            # Compute next target Q-values
            target_q1_values_next = self.target_q1_net.forward(states_next_,
                                                             actions_next_)
            target_q2_values_next = self.target_q2_net.forward(states_next_,
                                                             actions_next_)
            
            # Note: dones_.shape = rewards_.shape = (batch_size,)
            target_q1_values_next[dones_.view(-1,1)] = 0.0
            target_q2_values_next[dones_.view(-1,1)] = 0.0
            target_q1_values = rewards_.view(-1,1) + self.hparams.gamma*target_q1_values_next
            target_q2_values = rewards_.view(-1,1) + self.hparams.gamma*target_q2_values_next
        
        # Compute critic loss
        critic_loss = F.mse_loss(q1_values, target_q1_values)+F.mse_loss(q2_values, target_q2_values)
        
        # Critic gradient step
        critic_optim.zero_grad()
        self.manual_backward(critic_loss)
        critic_optim.step()
        self.critic_loss = float(critic_loss.cpu().detach().numpy())
        
        # Update of target network
        ## To do: Use target_tau
        if self.global_step % self.hparams.n_policy_upd_steps == 0:
            
            # Compute policy loss
            new_actions_ = self.policy.forward(states_)
            policy_loss = self.q1_net.forward(states_, new_actions_)
            ...
            policy_optim.step()
            self.policy_loss = float(policy_loss.cpu().detach().numpy())
            
            # Update Q-net net parameters
            for param, target_param in zip(self.q1_net.parameters(), self.target_q1_net.parameters()):
                target_param.data.copy_(self.hparams.target_tau * param.data\
                                        + (1 - self.hparams.target_tau) * target_param.data)
