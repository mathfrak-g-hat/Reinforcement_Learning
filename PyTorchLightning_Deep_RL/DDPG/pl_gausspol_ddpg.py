#######################################
##### DDPG WITH A GAUSSIAN POLICY #####
#######################################
### AJ Zerouali, 23/07/07
## gym v0.24.0 version
## Mostly a test for the GaussianPolicy nn.Module class.
...
class GaussPolicy_DDPG(LightningModule):
    ###################
    ### Constructor ###
    ###################
    def __init__(self,
    ...
    def build_networks(self)->None:
        '''
            Builds networks by calling MLP and GaussianPolicy
            class constructor.
        '''
        ...
        self.policy = GaussianPolicy(state_space_dim = self.state_space_dim,
                                     action_space_dim = self.action_space_dim,
                                     **self.hparams.policy_arch_dict)
        '''
        ### 23/07/05: Setting aside the action noise for now
        if self.hparams.action_noise_params == None:
            ...
            if self.hparams.action_noise_params["action_noise_class"] in [OrnsteinUhlenbeckActionNoise,
                ...
                self.get_action = self.eval_policy_add_noise #### IMPORTANT: IMPLEMENT THIS ####
    ...
    def get_action(self, state: th.Tensor, device, deterministic: bool = False):
        '''
             ...
             the output of the mu network of the Gaussian policy.s
        ...
        with th.no_grad():
            # Deterministic case
            if deterministic:
                action_ = self.policy.forward_mu(state_).to(device)
            # Stochastic case
            else:
                action_ = self.policy(state_).sample().to(device)
    ...
    def populate(self, n_steps: int = 1000) -> None:
        ...
        for i in range(n_steps):
            action = self.get_action(self.state, device = self.device, deterministic = False)
    ...
    def training_step(self, batch: Tuple[Tensor, Tensor], _) -> OrderedDict:
        ...
        actions_next_ = self.policy.forward(states_next_).sample() # sample() or rsample()?
        ...
        new_actions_ = self.policy.forward(states_).rsample()
